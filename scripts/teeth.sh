...
"force-split-at-every-level${SEP}src/tree.rs${SEP}        let force_split_root = if level == 0 {${SEP}        let force_split_root = if true {${SEP}force_split_is_only_considered_at_the_top" \
...
"tilings-skip-the-last-factor${SEP}src/shaping.rs${SEP}    for cols in 1..=number_of_macros {${SEP}    for cols in 1..number_of_macros {${SEP}every_factorisation_that_fits_is_a_tiling_in_column_order" \
"tilings-ignore-non-divisors${SEP}src/shaping.rs${SEP}        if number_of_macros % cols != 0 {${SEP}        if false {${SEP}every_factorisation_that_fits_is_a_tiling_in_column_order" \
"tiling-fit-checks-width-only${SEP}src/shaping.rs${SEP}        if w <= outline.x_max - outline.x_min && h <= outline.y_max - outline.y_min {${SEP}        if w <= outline.x_max - outline.x_min {${SEP}a_tiling_must_fit_in_BOTH_dimensions" \
"tiling-fit-is-strict${SEP}src/shaping.rs${SEP}        if w <= outline.x_max - outline.x_min && h <= outline.y_max - outline.y_min {${SEP}        if w < outline.x_max - outline.x_min && h < outline.y_max - outline.y_min {${SEP}a_tiling_exactly_filling_the_outline_is_kept" \
"tiling-rows-and-cols-swapped${SEP}src/shaping.rs${SEP}        let (w, h) = (cols * macro_width, rows * macro_height);${SEP}        let (w, h) = (rows * macro_width, cols * macro_height);${SEP}every_factorisation_that_fits_is_a_tiling_in_column_order" \
"no-retry-with-one-more-macro${SEP}src/shaping.rs${SEP}    if tilings.is_empty() {\n        tilings = generate_tilings_for_macro_cluster(${SEP}    if false {\n        tilings = generate_tilings_for_macro_cluster(${SEP}when_nothing_fits_the_search_is_retried_with_one_more_macro" \
"retry-runs-unconditionally${SEP}src/shaping.rs${SEP}    let mut tilings =\n        generate_tilings_for_macro_cluster(macro_width, macro_height, number_of_macros, outline);\n    if tilings.is_empty() {${SEP}    let mut tilings: Vec<Tiling> = Vec::new();\n    if tilings.is_empty() {${SEP}the_retry_is_only_reached_when_the_first_search_found_nothing" \
"unshapeable-reports-the-retried-count${SEP}src/shaping.rs${SEP}        return Err(Unshapeable { macro_width, macro_height, number_of_macros });${SEP}        return Err(Unshapeable { macro_width, macro_height, number_of_macros: number_of_macros + 1 });${SEP}a_cluster_that_fits_neither_count_is_unshapeable" \
